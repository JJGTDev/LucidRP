...
TriggerEvent('esx_phone:registerNumber', 'coopers', "Cooper Imports", false, false)
TriggerEvent('esx_society:registerSociety', 'coopers', "Cooper Imports", 'society_coopers', 'society_coopers', 'society_coopers', {type = 'private'})
...
AddEventHandler('esx_vehicleshop2:sellVehicle', function (vehicle)
	MySQL.Async.fetchAll('SELECT * FROM coopers_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		['@vehicle'] = vehicle
	}, function (result)
		local id = result[1].id

		MySQL.Async.execute('DELETE FROM coopers_vehicles WHERE id = @id', {
			['@id'] = id
		})
	end)
end)
...
AddEventHandler('esx_vehicleshop2:getStockItem', function (itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_coopers', function (inventory)
...
ESX.RegisterServerCallback('esx_vehicleshop2:buyVehicleSociety', function (source, cb, society, vehicleModel)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_' .. society, function (account)
		if account.money >= vehicleData.price then
			...
			MySQL.Async.execute('INSERT INTO coopers_vehicles (vehicle, price) VALUES (@vehicle, @price)', {
...
ESX.RegisterServerCallback('esx_vehicleshop2:getCommercialVehicles', function (source, cb)
	MySQL.Async.fetchAll('SELECT * FROM coopers_vehicles ORDER BY vehicle ASC', {}, function (result)
		local vehicles = {}
...
AddEventHandler('esx_vehicleshop2:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM coopers_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			local id    = result[1].id
			local price = ESX.Math.Round(result[1].price * 1.00)

			TriggerEvent('esx_addonaccount:getSharedAccount', 'society_coopers', function(account)
			...
			TriggerClientEvent('esx:showNotification', _source, _U('vehicle_sold2_for', vehicleModel, ESX.Math.GroupDigits(price)))
		else
