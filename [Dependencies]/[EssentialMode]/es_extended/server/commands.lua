TriggerEvent('es:addGroupCommand', 'tp', 'admin', function(source, args, user)
	...
	if x and y and z then
		...
		TriggerClientEvent('chat:addMessage', source, { template = '<div class="chat-message-system"><b>{0}:</b> {1}</div>',args = { 'System', 'Invalid coordinates!' } })
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { template = '<div class="chat-message-system"><b>{0}:</b> {1}</div>',args = { 'System', 'Insufficient Permissions.' } })
	...
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(args[2], args[3]) then
				...
				TriggerClientEvent('chat:addMessage', source, { template = '<div class="chat-message-system"><b>{0}:</b> {1}</div>',args = { 'System', 'That job does not exist.' } })
			end

		else
			TriggerClientEvent('chat:addMessage', source, { template = '<div class="chat-message-system"><b>{0}:</b> {1}</div>',args = { 'System', 'Player not online.' } })
		end
	else
		TriggerClientEvent('chat:addMessage', source, { template = '<div class="chat-message-system"><b>{0}:</b> {1}</div>',args = { 'System', 'Invalid usage.' } })
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { template = '<div class="chat-message-system"><b>{0}:</b> {1}</div>',args = { 'System', 'Insufficient Permissions.' } })
...
TriggerEvent('es:addGroupCommand', 'accmodify', 'admin', function(source, args, user)
	...
	if target and money_type and money_amount and xPlayer ~= nil then
		...
		elseif money_type == 'black' then
			...
			TriggerClientEvent('chat:addMessage', _source, { template = '<div class="chat-message-system"><b>{0}:</b> {1}</div>',args = { 'System', "^2" .. money_type .. " ^0 is not a valid money type!" } })
			return
		end
	else
		TriggerClientEvent('chat:addMessage', _source, { template = '<div class="chat-message-system"><b>{0}:</b> {1}</div>',args = { 'System', "Invalid arguments1." } })
		return
	end

	--print('es_extended: ' .. GetPlayerName(source) .. ' just set $' .. money_amount .. ' (' .. money_type .. ') to ' .. xPlayer.name)
	TriggerEvent('DiscordBot:ToDiscord', 'adminself', GetPlayerName(source) .. ' just set money', GetPlayerName(source) .. ' just set $' .. money_amount .. ' (' .. money_type .. ') to ' .. xPlayer.name, 'IMAGE_URL', true)
...
TriggerEvent('es:addGroupCommand', 'giveaccountmoney', 'admin', function(source, args, user)
	...
	if amount ~= nil then
		if xPlayer.getAccount(account) ~= nil then
			...
			TriggerEvent('DiscordBot:ToDiscord', 'adminself', GetPlayerName(source) .. ' just gave money', GetPlayerName(source) .. ' just gave $' .. amount .. ' (' .. account .. ') to ' .. xPlayer.name, 'IMAGE_URL', true)
		else
...
TriggerEvent('es:addGroupCommand', 'giveitem', 'admin', function(source, args, user)
	...
	if count ~= nil then
		if xPlayer.getInventoryItem(item) ~= nil then
			...
			TriggerEvent('DiscordBot:ToDiscord', 'adminself', GetPlayerName(source) .. ' just gave an item', GetPlayerName(source) .. ' just gave ' .. item .. ' (' .. count .. ') to ' .. xPlayer.name, 'IMAGE_URL', true)
		else
...
TriggerEvent('es:addGroupCommand', 'giveweapon', 'admin', function(source, args, user)
	...
	TriggerEvent('DiscordBot:ToDiscord', 'adminself', GetPlayerName(source) .. ' just gave a weapon', GetPlayerName(source) .. ' just gave a ' .. weaponName .. ' to ' .. xPlayer.name, 'IMAGE_URL', true)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { template = '<div class="chat-message-system"><b>{0}:</b> {1}</div>',args = { 'System', 'Insufficient Permissions.' } })
