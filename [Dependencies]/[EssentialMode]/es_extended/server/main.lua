	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		table.insert(tasks, function(cb)
			...
			table.insert(tasks2, function(cb2)
				...
				MySQL.Async.fetchAll('SELECT job, job_grade, loadout, position, firstname, lastname FROM `users` WHERE `identifier` = @identifier', {
					['@identifier'] = player.getIdentifier()
				}, function(result)
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				TriggerClientEvent('esx:showNotification', _source, _U('ex_inv_lim', targetXPlayer))
			else
				sourceXPlayer.removeInventoryItem(itemName, itemCount)
				targetXPlayer.addInventoryItem   (itemName, itemCount)
				
				TriggerClientEvent('esx:showNotification', _source, _U('gave_item', itemCount, ESX.Items[itemName].label, targetXPlayer.source))
				TriggerClientEvent('esx:showNotification', target,  _U('received_item', itemCount, ESX.Items[itemName].label, sourceXPlayer.source))
			end
	...
	elseif type == 'item_money' then
      ...
      --ADD DISCORD NOTIFICATION HERE LIKE EXAMPLE
	    --TriggerEvent('esx:givemoneyalert',sourceXPlayer.name,targetXPlayer.name,itemCount)
			TriggerClientEvent('esx:showNotification', _source, _U('gave_money', ESX.Math.GroupDigits(itemCount), targetXPlayer.source))
			TriggerClientEvent('esx:showNotification', target,  _U('received_money', ESX.Math.GroupDigits(itemCount), sourceXPlayer.source))
		else
	...
	elseif type == 'item_account' then
		...
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			TriggerClientEvent('esx:showNotification', _source, _U('gave_account_money', ESX.Math.GroupDigits(itemCount), Config.AccountLabels[itemName], targetXPlayer.source))
			TriggerClientEvent('esx:showNotification', target,  _U('received_account_money', ESX.Math.GroupDigits(itemCount), Config.AccountLabels[itemName], sourceXPlayer.source))
		else
			TriggerClientEvent('esx:showNotification', _source, _U('imp_invalid_amount'))
		end
	...
	elseif type == 'item_weapon' then
        ...
        --TriggerEvent('esx:giveweaponalert',sourceXPlayer.name,targetXPlayer.name,weaponLabel)
				TriggerClientEvent('esx:showNotification', _source, _U('gave_weapon_ammo', weaponLabel, itemCount, targetXPlayer.source))
				TriggerClientEvent('esx:showNotification', target,  _U('received_weapon_ammo', weaponLabel, itemCount, sourceXPlayer.source))
			else
        --ADD DISCORD NOTIFICATION HERE
        --TriggerEvent('esx:giveweaponalert',sourceXPlayer.name,targetXPlayer.name,weaponLabel)
				TriggerClientEvent('esx:showNotification', _source, _U('gave_weapon', weaponLabel, targetXPlayer.source))
				TriggerClientEvent('esx:showNotification', target,  _U('received_weapon', weaponLabel, sourceXPlayer.source))
			end

		else
			TriggerClientEvent('esx:showNotification', _source, _U('gave_weapon_hasalready', targetXPlayer, weaponLabel))
			TriggerClientEvent('esx:showNotification', target, _U('received_weapon_hasalready', sourceXPlayer, weaponLabel))
		end
	...
	if type == 'item_standard' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
			  ...
			  --ADD DISCORD NOTIFICATION HERE
        --TriggerEvent('esx:itemdropalert',sourceXPlayer.name,ESX.Items[itemName].label,itemCount)
      end
		end
