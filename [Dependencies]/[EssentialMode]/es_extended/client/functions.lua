...
ESX.Game.SpawnVehicle = function(modelName, coords, heading, cb)
	...
	Citizen.CreateThread(function()
		...
		local _JobPlate = GetVehicleNumberPlateText(vehicle)
		--TriggerServerEvent("TCS_fuel:setFuelForVehicle", _JobPlate, 100.0)
		exports["frenzy_fuel"]:SetFuel(vehicle, 100)
		RequestCollisionAtCoord(coords.x, coords.y, coords.z)
...
ESX.Game.GetPlayersInArea = function(coords, area, useZ)
	local players       = ESX.Game.GetPlayers()
	local playersInArea = {}
	if useZ == nil then
		useZ = false
	...
	for i=1, #players, 1 do
		local target       = GetPlayerPed(players[i])
		local targetCoords = GetEntityCoords(target)
		local distance     = GetDistanceBetweenCoords(targetCoords, coords.x, coords.y, coords.z, useZ)
                ...
                for i=1, #players, 1 do
                    if players[i] ~= PlayerId() then
						...
						ESX.TriggerServerCallback('esx:getOtherPlayerData', function (xPlayer)
							PlayerRPName = xPlayer.firstname.." "..xPlayer.lastname
						end, GetPlayerServerId(players[i]))

						while PlayerRPName == nil do
							Citizen.Wait(20)
						end
						if PlayerRPName == nil or PlayerRPName == "" then
							PlayerRPName = GetPlayerName(GetPlayerServerId(players[i]))
						end
                        table.insert(elements, {
                            label = PlayerRPName,
                            player = players[i]
