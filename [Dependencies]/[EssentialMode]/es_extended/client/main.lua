	...
	while true do
		...
		if IsControlJustPressed(1, 344) then
			if displayVal == 1 then
				displayVal = 0
			elseif displayVal == 0 then
				displayVal = 1
			end
			TriggerEvent("esx:setHUDVis")
		end
	end
end)
...
RegisterNetEvent("esx:setHUDVis")
AddEventHandler("esx:setHUDVis", function()
	ESX.UI.HUD.SetDisplay(displayVal)
	TriggerEvent('es:setMoneyDisplay', displayVal)
end)
...
AddEventHandler('esx:spawnVehicle', function(model)
	...
	ESX.Game.SpawnVehicle(model, coords, 90.0, function(vehicle)
		...
		exports["frenzy_hotwire"]:IsVehicleJob(vehicle, plate, true)
	end)
end)
...
AddEventHandler('esx:pickup', function(id, label, player)
	...
	ESX.Game.SpawnLocalObject('prop_paper_bag_01', {
		x = x,
		y = y,
		z = z - 2.0,
	}, function(obj)
...
AddEventHandler('esx:pickupWeapon', function(weaponPickup, weaponName, ammo)
	local playerPed = PlayerPedId()
	local pickupCoords = GetOffsetFromEntityInWorldCoords(playerPed, 1.5, 1.0, 0.5)
