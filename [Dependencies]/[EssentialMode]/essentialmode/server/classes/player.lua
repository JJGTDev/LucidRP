...
-- OOP hack
function CreatePlayer(source, permission_level, money, bank, identifier, license, group, roles, charfname, charlname)
	local self = {}
	...
	self.firstname = charfname
	self.lastname = charlname
	...
	rTable.addMoney = function(m)
		if type(m) == "number" then
			...
			TriggerEvent('DiscordBot:ToDiscord', 'cash', fal2.." Gained Cash", "$"..m.." To make a new balance of $"..newMoney, 'IMAGE_URL', true)

			if m > 950000 then
				local identifier = GetPlayerIdentifiers(self.source)[1]
				local name = GetPlayerName(self.source)
				fal2 = name.." "..identifier
				TriggerEvent('DiscordBot:ToDiscord', 'cheat', fal2.." Possible cheater", "$"..m.." Hit max threshold", 'IMAGE_URL', true)
			end
			...
			if m > 50000 then
				...
				local name = GetPlayerName(self.source)
				fal2 = name.." "..identifier
				TriggerEvent('DiscordBot:ToDiscord', 'largecash', fal2.." Possible cheater", "$"..m.." Large Money Transaction", 'IMAGE_URL', true)
			end
			-- Checks what money UI component is enabled
			if settings.defaultSettings.nativeMoneySystem == "0" then
				TriggerClientEvent('es:activateMoney', self.source , self.money)
	...
	rTable.removeMoney = function(m)
		if type(m) == "number" then
			...
			if settings.defaultSettings.nativeMoneySystem == "0" then
				...
				local name = GetPlayerName(self.source)
				fal2 = name.." "..identifier
				TriggerEvent('DiscordBot:ToDiscord', 'cash', fal2.." Lost Cash", "-$"..m.." To make a new balance of $"..newMoney, 'IMAGE_URL', true)
				if m > 950000 then
					local identifier = GetPlayerIdentifiers(self.source)[1]
					local name = GetPlayerName(self.source)
					fal2 = name.." "..identifier
	...
	rTable.addBank = function(m)
		if type(m) == "number" then
			...
			local name = GetPlayerName(self.source)
			fal2 = name.." "..identifier
			TriggerEvent('DiscordBot:ToDiscord', 'bank', fal2.." Gained Bank", "$"..m.." To make a new balance of $"..newBank, 'IMAGE_URL', true)
			if m > 950000 then
				local identifier = GetPlayerIdentifiers(self.source)[1]
				local name = GetPlayerName(self.source)
				fal2 = name.." "..identifier
				TriggerEvent('DiscordBot:ToDiscord', 'cheat', fal2.." Possible cheater", "$"..m.." Hit max bank deposit threshold", 'IMAGE_URL', true)
			end
			...
			if m > 50000 then
				...
				local name = GetPlayerName(self.source)
				fal2 = name.." "..identifier
				TriggerEvent('DiscordBot:ToDiscord', 'largecash', fal2.." Possible cheater", "$"..m.." Large bank Transaction", 'IMAGE_URL', true)
			end
		else
	...
	rTable.removeBank = function(m)
		if type(m) == "number" then
			...
			local name = GetPlayerName(self.source)
			fal2 = name.." "..identifier
			TriggerEvent('DiscordBot:ToDiscord', 'bank', fal2.." Lost Bank", "-$"..m.." To make a new balance of $"..newBank, 'IMAGE_URL', true)
			if m > 950000 then
				local identifier = GetPlayerIdentifiers(self.source)[1]
				local name = GetPlayerName(self.source)
				fal2 = name.." "..identifier
				TriggerEvent('DiscordBot:ToDiscord', 'cheat', fal2.." Possible cheater", "$"..m.." Hit max bank removal threshold", 'IMAGE_URL', true)
			end
