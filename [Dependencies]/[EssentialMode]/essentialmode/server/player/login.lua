...
function LoadUser(identifier, source, new, licenseNotRequired)
	...
	db.retrieveUser(identifier, function(user, isJson)
		...
		if user.license or licenseNotRequired then
			...
			Users[source] = CreatePlayer(source, user.permission_level, user.money, user.bank, user.identifier, user.license, user.group, user.roles or "", user.firstname, user.lastname)
