...
AddEventHandler('esx_outlawalert:outlawNotify', function(alert)
	if isPlayerWhitelisted then
		...
		TriggerEvent('chat:addMessage', { template = '<div class="chat-message-outlaw"><b>{0} |</b> {1}</div>', args = {'Dispatch', alert} })
	end
end)
...
Citizen.CreateThread(function()
	while true do
			...
			if vehicle and ((isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted) then
				...
				ESX.TriggerServerCallback('esx_outlawalert:isVehicleOwner', function(owner)
					if not owner then
						...
						}, streetName, vehicleClass, playerGender)
					end
				end, plate)
			end
		...
		elseif IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and Config.GunshotAlert and not IsBlacklistedWeapon(GetSelectedPedWeapon(playerPed)) then
