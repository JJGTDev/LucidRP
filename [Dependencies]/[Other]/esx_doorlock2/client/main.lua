...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_doorlock2:getDoorInfo', function(doorInfo)
			...
			if distance < maxDistance then
				...
				if IsControlJustReleased(0, 38) then
					if isAuthorized then
						...
						TriggerServerEvent('esx_doorlock2:updateState', k, doorID.locked) -- Broadcast new state of the door to everyone
					end
				end
			end
		end
...
RegisterNetEvent('esx_doorlock2:setState')
AddEventHandler('esx_doorlock2:setState', function(doorID, state)
	Config.DoorList[doorID].locked = state
end)
