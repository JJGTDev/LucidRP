...
AddEventHandler('esx_doorlock2:updateState', function(doorID, state)
	...
	if type(state) ~= 'boolean' then
		print(('esx_doorlock2: %s attempted to update invalid state!'):format(xPlayer.identifier))
		return
	end

	if not Config.DoorList[doorID] then
		print(('esx_doorlock2: %s attempted to update invalid door!'):format(xPlayer.identifier))
		return
	end

	if not IsAuthorized(xPlayer.job.name, Config.DoorList[doorID]) then
		print(('esx_doorlock2: %s was not authorized to open a locked door!'):format(xPlayer.identifier))
		return
	end
