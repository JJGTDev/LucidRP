...
$("#banbtn").click(function() {
    ...
    if (target == 0 || target == "" || reason == "" || (length == "" && !perm)) {
        ...
        $.post("http://frenzy_admin/ban", JSON.stringify({ target: target, reason: reason, length: perm ? null : length, offline: offline }));
    }
});
...
$("#warnbtn").click(function() {
    ...
    if (target == 0 || target == "" || message == "") {
        ...
        $.post("http://frenzy_admin/warn", JSON.stringify({ target: target, message: message, anon: anon }));
    }
});
...
$("body").on("click", "#unbanbtn", function() {
    $.post("http://frenzy_admin/unban", JSON.stringify({ id: $(this).data("id") }));
...
$(function() {
    ...
    document.onkeyup = function(data) {
        if (data.which == 27) {
            toggle();
            $.post("http://frenzy_admin/hidecursor", JSON.stringify({}));
        }
    };
