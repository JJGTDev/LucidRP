...
function TogglePositionType()
    if ( radarInfo.positionType == "right-side" ) then
        radarInfo.positionType = "right-side-middle"
        Notify( "~b~Position set to middle right side" )
    elseif ( radarInfo.positionType == "right-side-middle" ) then
        radarInfo.positionType = "none"
        Notify( "~b~Position set to middle" )
    else
        radarInfo.positionType = "right-side"
        Notify( "~b~Position set to right side" )
    end
end
        ...
        if ( IsPedSittingInAnyVehicle( ped ) ) then 
            ...
            if ( GetPedInVehicleSeat( vehicle, -1 ) == ped and GetVehicleClass( vehicle ) == 18 ) then 
                ...
                SendNUIMessage({
                    ...
                    positionType = radarInfo.positionType
                })
            end 
        end 
    end 
...
RegisterNUICallback( "RadarRC", function( data, cb ) 
    ...
    elseif ( data == "radar_speedtype" ) then 
     ...
     -- Speed Type 
    elseif ( data == "radar_positionType" ) then 
        TogglePositionType()
...
Citizen.CreateThread( function()
    while true do 
        ...
        if ( IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 45 ) ) then 
            TriggerEvent( 'wk:radarRC' )
        end 

        -- LCtrl is not being pressed and M has just been pressed 
        if ( not IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 45 ) ) then 
            ResetFrontFast()
            ResetRearFast()
        end 
